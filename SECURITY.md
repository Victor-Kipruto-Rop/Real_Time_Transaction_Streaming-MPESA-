...
2. **Idempotency**
   ...
   - Enforce a unique constraint on `TransID` at the database layer
   - Use `TransID` as the Kafka message key so duplicates land on the same partition
   - Return 200 OK on a detected duplicate so Safaricom stops retrying
...
3. **Pipeline & Operational Monitoring**
   ...
   - Alert on dbt run failures via Airflow `on_failure_callback`
   - Route alerts to a monitored channel (Slack/email), not just Grafana's alert list
