    ...
    def _init_pool(self):
        ...
        try:
            if self.use_iam_auth:
                ...
                host = (
                    self._explicit_config["host"]
                    or os.environ.get("POSTGRES_HOST", "localhost")
                )
                port = (
                    self._explicit_config["port"]
                    or int(os.environ.get("POSTGRES_PORT", "5432"))
                )
                database = (
                    self._explicit_config["database"]
                    or os.environ.get("POSTGRES_DB", "mpesa_analytics")
                )
                user = (
                    self._explicit_config["user"]
                    or os.environ.get("POSTGRES_USER", "data_engineer")
                )
                password = (
                    self._explicit_config["password"]
            ...
            logger.info(
                f"✓ Connection pool initialized ({self.min_connections}-{self.max_connections})"
            )
