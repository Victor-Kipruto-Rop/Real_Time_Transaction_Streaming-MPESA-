    ...
    def test_sql_injection_in_phone_number(self, test_client):
        ...
        for payload in malicious_payloads:
            response = test_client.get(
                "/api/v1/transactions", params={"phone_number": payload}
            )
...
class TestHMACSignatureVerification:
    ...
    def test_missing_signature_header(self, test_client):
        ...
        # No signature header
        response = test_client.post("/api/v1/webhooks/c2b/confirmation", json=payload)
    ...
    def test_tampered_payload_detection(self, test_client):
        ...
        signature = hmac.new(b"test-secret", body.encode(), hashlib.sha256).hexdigest()
        ...
        response = test_client.post(
            "/api/v1/webhooks/c2b/confirmation", json=tampered_payload, headers=headers
        )
...
class TestXSSPrevention:
    ...
    def test_xss_in_transaction_reference(self, test_client):
        ...
        response = test_client.post("/api/v1/transactions/initiate-stk", json=payload)
...
class TestSensitiveDataExposure:
    ...
    def test_error_messages_dont_leak_info(self, test_client):
        ...
        # Trigger 500 error
        response = test_client.get("/api/v1/transactions/invalid-id-12345")
        ...
        assert (
            "database" not in response_text or "database.html" in response_text
        )  # Allow 404 page
    ...
    def test_api_response_no_pii_in_logs(self, test_client):
        ...
        response = test_client.get(
            "/api/v1/transactions", params={"phone_number": "254712345678"}
        )
...
class TestAuthenticationAuthorization:
    ...
    def test_customer_data_isolation(self, test_client):
        ...
        response = test_client.get(
            "/api/v1/transactions", params={"phone_number": "254712345678", "limit": 10}
        )
...
class TestInputValidation:
    ...
    def test_phone_number_validation(self, test_client):
        ...
        for phone in invalid_phones:
            response = test_client.get(
                "/api/v1/transactions", params={"phone_number": phone}
            )
    ...
    def test_amount_validation(self, test_client):
        ...
        for amount in invalid_amounts:
            ...
            response = test_client.post(
                "/api/v1/transactions/initiate-stk", json=payload
            )
    ...
    def test_date_validation(self, test_client):
        ...
        for date in invalid_dates:
            response = test_client.get(
                "/api/v1/transactions", params={"start_date": date}
            )
...
class TestSecurityHeaders:
    ...
    def test_security_headers_present(self, test_client):
        ...
        assert "Server" not in response.headers or "Python" not in response.headers.get(
            "Server", ""
        )
