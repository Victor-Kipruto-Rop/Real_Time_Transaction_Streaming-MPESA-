    ...
    def test_producer_initialization(self, mock_env_vars):
        ...
        with patch("kafka.KafkaProducer") as mock_producer:
            producer = KafkaProducerClient(
                bootstrap_servers="localhost:9092", topic="test-transactions"
            )
            assert producer is not None
            mock_producer.assert_called_once()

    def test_send_transaction_message(
        self, mock_kafka_producer, sample_mpesa_transaction
    ):
...
class TestKafkaDLQ:
    ...
    def test_dlq_retry_logic(self, mock_kafka_producer):
        ...
        dlq_message = {"TransID": "TXN123", "retry_count": 2, "max_retries": 3}
    ...
    def test_lag_monitoring(self, mock_env_vars):
        ...
        with patch("kafka.KafkaConsumer") as mock_consumer:
            ...
            lag = metrics.get_consumer_lag(
                consumer=mock_consumer_instance, topic="test-transactions"
            )
